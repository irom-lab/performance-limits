...
def gen_traj(x_path, y_path, z_path, T, dt):
    ...
    for i in range(x_traj.shape[0] - 1):
        heading_direction = np.array([x_traj[i + 1], y_traj[i + 1], z_traj[i + 1]]) - np.array(
            [x_traj[i], y_traj[i], z_traj[i + 1]])
...
def gen_end_points(x_min, x_max, z_min, z_max, num_x_points=5, num_z_points=1):

    end_points = np.empty([num_x_points * num_z_points, 2], dtype=float)
    x_points = np.linspace(x_min, x_max, num_x_points)

    # Flip end-points so that it's right to left (to match LIDAR)
...
def gen_prim_lib(x_min, x_max, z_min=1, z_max=1, num_x_points=5, num_z_points=1, vy=1.25, dt=0.05, T=1, steps=4000):
    '''
    Generates the library of primitives.
    Ensure that vy * T = 1.25 m
    '''
    num_prims = num_x_points * num_z_points
    end_points = gen_end_points(x_min=-1,
                                x_max=1,
                                z_min=1,
                                z_max=1,
    ...
    for i in range(num_prims):
        ...
        x_traj, y_traj, z_traj, thetax_traj, thetay_traj, thetaz_traj, v = gen_traj(x_path, y_path, z_path, T=T, dt=dt)
...
if __name__ == '__main__':
    ...
    num_primitives = 10
    prim_lib_x_traj, prim_lib_y_traj, prim_lib_z_traj, prim_lib_x_acc, prim_lib_y_acc, prim_lib_z_acc, prim_lib_v = gen_prim_lib(x_min=-1, x_max=1, num_x_points=num_primitives) # , z_min=-1, z_max=1, num_x_points=5, num_z_points=5, dt=dt, T=T)

    # All primitives
    num_points_per_primitive = prim_lib_x_traj.shape[1]
    prim_lib = np.zeros((2, num_primitives, num_points_per_primitive))
    prim_lib[0, :, :] = prim_lib_x_traj
    prim_lib[1, :, :] = prim_lib_y_traj
    # prim_lib[2,:,:] = prim_lib_z_traj

    # Plot primitives in x-y space
    for i in range(num_primitives):    # primitive number i in library
        plt.scatter(prim_lib[0, i, :], prim_lib[1, i, :])

    plt.show()

    # Save library
    np.save('lbi/primitives/primitive_library.npy', prim_lib)
    print("Saved motion primitives.")
