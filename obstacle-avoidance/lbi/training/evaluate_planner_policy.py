...
def main(raw_args=None):
    ...
    # LIDAR points
    angles = np.linspace(lidar_angle_range[0], lidar_angle_range[1], num_rays)
    ray_points = np.array([np.cos(angles), np.sin(angles)])
    ...
    for i in range(num_test_envs):
        ...
        # ipy.embed()
        lidar_detected_points = ray_points * lidar_dists.numpy()
        dists = get_primitive_dists(lidar_detected_points, prim_lib)
        pred_primitive = prim_lib[:,np.argmax(dists),:]
