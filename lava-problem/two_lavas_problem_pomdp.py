...
class TransitionModel(pomdp_py.TransitionModel):
    def __init__(self, goal, lava1, lava2, num_states):
        self.goal = goal
        self.lava1 = lava1
        self.lava2 = lava2
    ...
    def probability(self, next_state, state, action):
        """Lava and goal are absorbing states. Otherwise, we move according to the action.
        ...
        if state.name == "dummy": # if we are in the initial dummy state
            if (next_state.name is self.lava1) or (next_state.name is self.lava2): # We don't start in lava
                return 0.0 
            else: # actual state is initialized uniformly in a non-lava state
                return 1/(self.num_states - 2) 

        else: # if we're not in the dummy initial state

            if (next_state.name==self.goal and state.name == self.goal): # the goal is absorbing
                return 1.0 
            if (next_state.name==self.lava1 and state.name == self.lava1): # the lava is absorbing
                return 1.0 
            if (next_state.name==self.lava2 and state.name == self.lava2): # the lava is absorbing
                return 1.0 

            j = int(state.name)
            i = int(next_state.name)
            goal = int(self.goal)
            lava1 = int(self.lava1)
            lava2 = int(self.lava2)

            if (action.name.startswith("left") and max(j-1,0)==i and (j is not goal) and (j is not lava1) and (j is not lava2)): # i is left of j
                return 1.0
            if (action.name.startswith("right") and min(j+1,self.num_states-1)==i and (j is not goal) and (j is not lava1) and (j is not lava2)): # i is right of j
                return 1.0
...
class RewardModel(pomdp_py.RewardModel):
    def __init__(self, reward_x, goal, lava1, lava2):
    ...
    def _reward_func(self, state, action):
        ...
        if (state.name == self.lava1) or (state.name == self.lava2):
...
class TwoLavasProblem(pomdp_py.POMDP):
    """
    Lava problem class.
    """

    def __init__(self, p_correct, reward_x, goal, lava1, lava2, num_states, init_true_state, init_belief):
        """init_belief is a Distribution."""
        agent = pomdp_py.Agent(init_belief,
                               PolicyModel(),
                               TransitionModel(goal, lava1,  lava2, num_states),
                               ObservationModel(p_correct, num_states),
                               RewardModel(reward_x, goal, lava1, lava2))
        env = pomdp_py.Environment(init_true_state,
                                   TransitionModel(goal, lava1, lava2, num_states),
                                   RewardModel(reward_x, goal, lava1, lava2))
        super().__init__(agent, env, name="TwoLavasProblem")
...
def main(raw_args=None):
    ...
    ##### Other params #####
    horizon = 5 # Time horizon (without dummy state)
    goal = "2" # goal state 
    lava1 = "4" # lava state
    lava2 = "0" # another lava state
    ...
    lava_problem = TwoLavasProblem(p_correct, reward_x, goal, lava1, lava2, num_states, init_true_state, init_belief)
