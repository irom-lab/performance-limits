...
def main(raw_args=None):
	...
	R = np.zeros((nx,nu)) # c_t(x,u)
	lava1 = nx-1 # Lava state (right-most state)
	lava2 = 0 # Another lava state (left-most state)
	goal = 2  # Goal state
	r_xs = reward_x*np.ones(nx) # Reward of being at any given (non-lava/goal) state
	r_xs[lava1] = 0.000001 # Reward of being in lava1
	r_xs[lava2] = 0.000001 # Reward of being in lava2
	...
	p0 = np.ones(nx)
	p0[lava1] = 0.0 # You cannot start in the lava1
	p0[lava2] = 0.0 # You cannot start in the lava2
	p0 = p0/sum(p0)
	...
	for i in range(nx):
		for j in range(nx):
			if (j==goal and i == goal): # the goal is absorbing
				...
				px_x[i,j,1] = 1.0
				continue
			if (j==lava1 and i == lava1): # lava1 is absorbing
				px_x[i,j,0] = 1.0
				px_x[i,j,1] = 1.0
				continue
			if (j==lava2 and i == lava2): # lava2 is absorbing
				px_x[i,j,0] = 1.0
				px_x[i,j,1] = 1.0
				continue

			if (max(j-1,0)==i and (j is not goal) and (j is not lava1) and (j is not lava2)): # i is left of j
				px_x[i,j,0] = 1.0
			if (min(j+1,nx-1)==i and (j is not goal) and (j is not lava1) and (j is not lava2)): # i is right of j
				px_x[i,j,1] = 1.0
