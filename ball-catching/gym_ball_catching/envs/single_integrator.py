"""
Single Integrator with noise in observations.
"""
...
class SingleIntegratorEnv(gym.Env):
    ...
    def __init__(self, noise_scale=0.1): 
        ...
        self.nx = 1 # Number of states
        self.ny = self.nx # Number of observations
        self.nu = 5 # Number of control inputs
        self.vel_mag = 10.0 # scaling for control input

        self.tau = 0.1 # Time step
        self.T = 3 # 5 # 10 # Time horizon
        ...
        self.noise_std_dev = self.noise_scale*np.array([1.0])


        # Setup Kalman filter
        self.kalman_filter = True
        self.x0_belief_std_dev = 1.0*np.array([self.x_threshold]) 
    ...
    def step(self, action):
        ...
        # u = self.vel_mag if action == 1 else -self.vel_mag
        if action == 0:
            u = 0.0
        elif action == 1:
            u = 0.5*self.vel_mag
    ...
    def reset(self):
        ...
        self.state = self.np_random.uniform(low=[self.x_range[0]], high=[self.x_range[1]])
        # self.state = self.np_random.uniform(low=[self.x_range[0]], high=[self.x_range[0]+1])
